...
let storesInitialized = null;
let walletTimestampsStore = null;
let transferCooldownStore = null;
let bindingsStore = null;
    ...
    if (!storesInitialized) {
        storesInitialized = (async () => {
            try {
                ...
                walletTimestampsStore = new Keyv({ store, namespace: "wallet-ts" });
                transferCooldownStore = new Keyv({ store, namespace: "cooldown" });
                bindingsStore = new Keyv({ store, namespace: "bindings" });
...
const verifyAuthorMintPass = async (props) => {
    ...
    const authorWallet = wallets["eth"];
    ...
    const lastTimestampOfAuthor = await walletTimestampsStore.get(cacheKey);
 ...
 */
const getAuthorWalletAddress = (publication) => {
    const wallets = publication.author.wallets || {};
    const authorWallet = wallets["eth"];
    if (typeof authorWallet?.address === "string") {
