...
async function postJson<T>(path: string, body: unknown, opts?: { timeoutMs?: number }): Promise<T> {
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && opts.timeoutMs > 0 ? opts.timeoutMs : 10000;
...
export default function RequestPage({
    ...
    const handleRouteChangeStart = (url: string) => {
      if (isVerificationInProgress && !url.startsWith("/request")) {
        const confirmed = window.confirm(
          "You have an SMS verification in progress. Are you sure you want to leave?",
        );
        if (!confirmed) {
  ...
  const isCountrySupported =
    !selectedCountry || !UNSUPPORTED_COUNTRIES.includes(selectedCountry as RPNInput.Country);
  ...
  async function handleCheckEligibilityAndSendCode(currentAddress: string) {
    try {
      ...
      await postJson<{
        ok: true;
        sid?: string;
        mode: "verify" | "kv-fallback";
        debugCode?: string;
      }>("/api/sms/send", { phoneE164: phone, address: currentAddress }, { timeoutMs: 15000 });
      setStep("code");
    } catch (e: unknown) {
      if (e instanceof Error) {
        const errorWithCooldown = e as Error & {
          status?: number;
          cooldownSeconds?: number;
          providerError?: {
            provider?: string;
        ...
        const providerCode = pe?.errorCode ?? pe?.code;
        const providerMessage = pe?.errorMessage ?? pe?.message;
        // Prefer provider message/code if present
        if (pe && (providerMessage || providerCode !== undefined)) {
          const prov = pe.provider ? `${pe.provider} ` : "";
          const codePart =
            providerCode !== undefined && providerCode !== null
              ? ` (code ${String(providerCode)})`
              : "";
          const detail = `${prov}${providerMessage || "delivery error"}${codePart}`;
          setError(`${e.message} — ${detail}`);
        } else if (
          errorWithCooldown.status === 429 &&
          typeof errorWithCooldown.cooldownSeconds === "number" &&
          errorWithCooldown.cooldownSeconds > 0
        ) {
  ...
  async function handleVerifyAndMint() {
    try {
      ...
      const mint = await postJson<{ ok: boolean; txHash?: string }>("/api/mint", {
            ...
            <div className="space-y-4">
              ...
              {isNorthAmerica ? (
                <p className="text-[0.5rem] text-muted-foreground">
                  ...
                  and consent to one-time phone verification via SMS. Message and data rates may
                  apply.
                </p>
              ) : (
                <p className="text-[0.5rem] text-muted-foreground">
