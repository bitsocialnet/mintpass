...
`AGENTS.md` provides shared project guidance; `CLAUDE.md` imports it. The committed skills are `impeccable`, for requested frontend design/refinement work, and `retro`, for evidence-backed recurrence prevention. Other skills may be available in a contributor’s app, but are not provided by this checkout.
